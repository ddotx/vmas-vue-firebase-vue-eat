...
// When cafeSource is updated
watch(cafeSource, (cafeSource) => {
    // Update the editCafe with the appropriate values
    editCafe.value = {
        ...cafeSource,
    }
})
            ...
            <BaseCard>
                <template v-slot:default>
                    <BaseForm v-if="editCafe">
                        <BaseInput v-model="editCafe.name" label="Name" required placeholder="Cafe with a Vue" />
                        <BaseInput v-model="editCafe.location" label="Location" required />
                        <BaseInput v-model.number="editCafe.price" label="Price" type="number" min="1" max="4" required />
                        <BaseInput v-model="editCafe.rating" label="Rating" type="number" min="0" max="5" step="0.5"
                            required />
